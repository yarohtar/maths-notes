...
f(x)=G(f|_{I_{x}})
$$
where $I_{x}$ is the [[Foundations/Ordinals/Initial Segment]].

### Theorem
Say $h$ is an attempt if $h$ is a function $I\to Y$ 
where the domain of $h$ is an [[Foundations/Ordinals/Initial Segment]] $I$ of $X$ and 
$$
...
We first show that if $h,h'$ are attempts, then $h(x)=h'(x)$ for all $x\in dom(h)\cap dom(h')$ by [[Foundations/Ordinals/Proof by Induction]].
...
$$
Now just check that this is a [[Foundations/Set Theory/Function]] and check that $f$ is an attempt.
