Let $S$ be a [[Foundations/Consistent]] [[Foundations/Theory]] in a first-order language $L=L\left( \Omega,\prod \right)$
Then $S$ has a model.
...
We turn this into a [[Foundations/Structure]] in the obvious way
...
1. Let $S$ be a consistent theory, $p$ be a sentence
   ...
   If not, then by [[Foundations/Deduction Theorem (First-order Logic)]] 
   $S\vdash \neg p$ and $S\vdash \neg \neg p$ and hence $S\vdash \bot$ by (MP)
   By [[Foundations/Zorn's Lemma]] there is a maximal consistent theory $\bar{S}\supseteq S$. 
...
2. Suppose $S\vdash(\exists x)p$ where $p$ is a formula with $FV(p)=\{ x \}$. Add a new constant $c$ to $L$. Then $S\cup \{ p[c/ x] \}$ is consistent
   If not, then by [[Foundations/Deduction Theorem (First-order Logic)]] $S\vdash \neg p[c /x]$
...
We make $A$ an $L$ [[Foundations/Structure]]:
