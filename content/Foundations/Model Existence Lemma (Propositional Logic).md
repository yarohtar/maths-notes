Let $S\subseteq L$ be [[Foundations/Consistent]].
Then $S$ has a [[Foundations/Model (Propositional Logic)]]
## Proof
...
Then by [[Foundations/Deduction Theorem (Propositional Logic)\|Deduction Theorem]], we have $T\vdash \neg t$ and $T\vdash(\neg t\implies \bot)$ and hence $T\vdash \bot$ by (MP) which is a contradiction
...
Let $C=\{ T_{i}:i\in I \}$ be a nonempty [[Foundations/Chain]] in $X$. 
We show $T=\bigcup_{i\in I}T_{i}$ is an [[Foundations/Upper Bound]]: 
...
By [[Foundations/Zorn's Lemma]], $X$ has a maximal element $\bar{S}$
Now for any $t\in L$, either $\bar{S}\cup \{ t \}$ or $\bar{S}\cup \{ \neg t \}$ is consistent,
otherwise $\bar{S}\vdash \neg t$ and $\bar{S}\vdash \neg \neg t$ 
by the [[Foundations/Deduction Theorem (Propositional Logic)]]
...
#### Case 3
$v(p)=0$ i.e. $p\not\in \bar{S}$ and so $\neg p \in \bar{S}$
need: $v(p \implies q)=1$, i.e. $\bar{S}\vdash(p \implies q)$ or equivalently $\bar{S}\cup \{ p \}\vdash q$ by [[Foundations/Deduction Theorem (Propositional Logic)\|Deduction Theorem]]
Note that:
$$
\begin{align}
p\quad %quad
