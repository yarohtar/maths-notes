Let $X$ be a (nonempty) [[Foundations/Poset/Poset]].
Suppose every [[Foundations/Poset/Chain]] in $X$ has an [[Foundations/Poset/Upper Bound]]. 
Then $X$ has a [[Foundations/Ordinals/Maximal]] element.
#### Proof
Assume $X$ has no maximal element. 
For each $x\in X$ fix $x'\in X$ s.t. $x<x'$ (by [[Foundations/Set Theory/Axiom of Choice]])
