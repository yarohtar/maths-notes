...
if there is a [[Foundations/Computable]] $Y\subseteq \mathbb{B}^{k+1}$ 
such that for all $\vec{w}\in \mathbb{B}^{k}$ we have
$$
\vec{w}\in X\iff \exists v((\vec{w},v)\in Y)
$$
### Theorem
A set is $\Sigma_{1}$ if and only if it is [[Foundations/Computably Enumerable]].
#### Proof
...
### Proposition
A set is [[Foundations/Computable]] if and only if it is a $\Delta_{1}$ set.
#### Proof
$(\implies)$ A set is $\Sigma_{1}$ iff it is [[Foundations/Computably Enumerable]]. 
It is easy to see that for a [[Foundations/Computable]] set, 
...
so $X$ will be [[Foundations/Computable]].