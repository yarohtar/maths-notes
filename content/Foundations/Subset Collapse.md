Let $Y$ be a [[Foundations/Well-ordered]] set and $X\subseteq Y$
Then there is a unique [[Foundations/Initial Segment]] of $Y$ which is [[Foundations/Order-isomorphic]] to $X$ 
...
$$
for all $x\in X$
By [[Foundations/Proof by Induction]], $f$ is uniquely determined 
##### Existence
WLOG $Y\neq \emptyset$
Fix $y_{0}\in Y$
...
It will then follow that $im(f)$ is an [[Foundations/Initial Segment]] of $Y$ and we are done.
