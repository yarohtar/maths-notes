...
#### Proof
##### $\implies$
Suppose we are given a [[Foundations/Category Theory/Functor]] $G:\mathcal{D}\to \mathcal{C}$ 
and [[Foundations/Category Theory/Natural Isomorphism]]s $1_{\mathcal{C}}\xrightarrow{\alpha} GF$ and $FG\xrightarrow{\beta}1_{\mathcal{D}}$.
For any $B\in \operatorname{ob}\mathcal{D}$, 
$\beta_{B}$ is an [[Foundations/Category Theory/Isomorphism]] $FGB\to B$ 
so $F$ is [[Foundations/Category Theory/Essentially Surjective]]
...
so we get $Ff=g$. 
##### $\impliedby$
For each $B\in \operatorname{ob}\mathcal{D}$ choose an [[Foundations/Category Theory/Object]] $GB$ of $\mathcal{C}$ 
and an [[Foundations/Category Theory/Isomorphism]] $\beta_{B}:FGB\to B$ in $\mathcal{D}$, 
which we can do as $F$ is [[Foundations/Category Theory/Essentially Surjective]].
Given $B\xrightarrow{g}C$ in $\mathcal{D}$ consider
$$
FGB\xrightarrow{\beta_{B}}B\xrightarrow{g}C\xrightarrow{\beta_{C}^{-1}}FGC
$$
and define $GB\xrightarrow{Gg}GC$ to be the unique [[Foundations/Category Theory/Morphism]] such that
$$
FGg=\beta_{C}^{-1}g\beta_{B}
$$
which we can do as $F$ is [[Foundations/Category Theory/Full]] (existence) and [[Foundations/Category Theory/Faithfull]] (uniqueness)

Uniqueness implies [[Foundations/Category Theory/Functor\|functoriality]]: 
...
Define $\alpha_{A}:A\to GFA$ to be the unique [[Foundations/Category Theory/Morphism]]
whose inverse under $F$ is 
$$
FA\xrightarrow{\beta_{FA}^{-1}}FGFB
$$
As we already saw, $\alpha_{A}$ is an [[Foundations/Category Theory/Isomorphism]] for all $A$, 
and [[Foundations/Category Theory/Naturality Square]]s for $\alpha$ are mapped by $F$ 
to naturality squares for $\beta ^{-1}$ so they commute.
 
### Example
[[Foundations/Category Theory/Category of Partial Functions]] $\mathrm{Part}$ is equivalent to 
...
\end{cases}
$$
Then $FG=1_{\mathrm{Part}}$ and there is a [[Foundations/Category Theory/Natural Isomorphism]] $1_{\mathrm{Set_{*}}}\to GF$ 
