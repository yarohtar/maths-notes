The [[Foundations/Covariant]] power set functor $A\to PA$ is a [[Foundations/Functor]] $\mathrm{Set}\to \mathrm{Set}$ [[Foundations/Category of Sets]]
Given $A\xrightarrow{f}B$ 
we define $Pf:PA\to PB$ by $Pf(A')=\{ f(a):a\in A' \}\subseteq B$

We also have the [[Foundations/Contravariant]] powerset functor:
