...
if there is a [[Foundations/Computability/Computable]] $Y\subseteq \mathbb{B}^{k+1}$ 
such that for all $\vec{w}\in \mathbb{B}^{k}$ we have
$$
\vec{w}\in X\iff \exists v((\vec{w},v)\in Y)
$$
### Theorem
A set is $\Sigma_{1}$ if and only if it is [[Foundations/Computability/Computably Enumerable]].
#### Proof
...
$(\impliedby)$ Suppose $X$ is [[Foundations/Computability/Computably Enumerable]]. 
Let $M$ be a machine that lists all members of $X$. 
Construct the set $Y$ such that $(\vec{w},v)\in Y$ 
iff $M$ lists $\vec{w}$ in step $\# v$. 
Then $Y$ is [[Foundations/Computability/Computable]], so $X$ is $\Sigma_{1}$.
### Proposition
The class of $\Sigma_{1}$ sets is not closed under complementation.
#### Proof
[[Foundations/Computability/The Halting Problem]]
# $\Pi_{1}$ sets
A set is called $\Pi_{1}$ if it is a complement of a $\Sigma_{1}$ set.
### Proposition
A set $X$ is $\Pi_{1}$ if and only if there is a [[Foundations/Computability/Computable]] $Y\subseteq \mathbb{B}^{k+1}$ 
such that for all $\vec{w}\in \mathbb{B}^{k}$ we have
$$
...
### Proposition
A set is [[Foundations/Computability/Computable]] if and only if it is a $\Delta_{1}$ set.
#### Proof
$(\implies)$ A set is $\Sigma_{1}$ iff it is [[Foundations/Computability/Computably Enumerable]]. 
It is easy to see that for a [[Foundations/Computability/Computable]] set, 
...
so $X$ will be [[Foundations/Computability/Computable]].