...
Given a [[Foundations/Computability/Register Machine]] $M$ and $k>0$, we can define a [[Foundations/Computability/Partial Function]] $f_{M,k}:\mathbb{B}^{k}\dashrightarrow \mathbb{B}$ by:
1. $f_{M,k}(\vec{w})\uparrow$ iff $M$ does not halt on $\vec{w}$
2. $f_{M,k}(\vec{w})=v_{0}$ iff $M$ halts on $\vec{w}$ with register content $\vec{v}$
Note that $f_{M,k}$ is necessarily [[Foundations/Computability/Computable]].
