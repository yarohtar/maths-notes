Let $L\subseteq \mathbb{W}$ be a [[Foundations/Computability/Formal Language]]
...
### Theorem
For every [[Foundations/Computability/Context-Free Formal Language]] $L$ 
there is a number $n$ 
such that $L$ satisfies the context free pumping lemma 
with pumping number $n$.
#### Proof (sketch)
WLOG $G$ generating $L$ is in [[Foundations/Computability/Chomsky Normal Form]].
Let $m=\lvert V \rvert$ and $n=2^m+1$. 
We claim $n$ is the pumping number of $G$.
Let $w\in L$ be a word with $\lvert w \rvert\geq n$
Note that the height of the [[Foundations/Computability/Parse Tree]] of $w$ is at least $m+1$. 
