A code $C\subseteq \mathbb{F}_{2}^{n}$ is a cyclic code if it is [[XNotes/Linear Code]] and 
$$
(a_{0},\dots,a_{n-1})\in C\implies(a_{1},\dots,a_{n-1},a_{0})\in C
$$
We identify $\mathbb{F}_{2}^{n}$ with the [[XNotes/Ring]] $\mathbb{F}_{2}[X] / (X^{n}-1)$ via
$$
...
### Corollary
$C$ is a cyclic code of length $n$ if and only if $\mathcal{C}$ is an [[XNotes/Ideal]] in $\mathbb{F}_{2}[X] / (X^{n}-1)$
We identify $C$ and $\mathcal{C}$.
### Lemma
$\mathbb{F}_{2}[X] /(X^{n}-1)$ is a [[XNotes/Principal Ideal Domain]]
Moreover, each of its ideals is generated by some $g$ such that $g(X)\mid X^{n}-1$
#### Proof
Note that $\mathbb{F}_{2}[X] /(X^{n}-1)$ is [[XNotes/Euclidean Domain]] 
so also [[XNotes/Principal Ideal Domain]].
...
[[XNotes/Generator Polynomial]]
[[XNotes/Parity Check Polynomial]]
### Proposition
