...
### Corollary
$C$ is a cyclic code of length $n$ if and only if $\mathcal{C}$ is an [[Algebra/Ring Theory/Ideal]] in $\mathbb{F}_{2}[X] / (X^{n}-1)$
We identify $C$ and $\mathcal{C}$.
### Lemma
$\mathbb{F}_{2}[X] /(X^{n}-1)$ is a [[Algebra/Ring Theory/Principal Ideal Domain]]
Moreover, each of its ideals is generated by some $g$ such that $g(X)\mid X^{n}-1$
#### Proof
Note that $\mathbb{F}_{2}[X] /(X^{n}-1)$ is [[Algebra/Ring Theory/Euclidean Domain]] 
so also [[Algebra/Ring Theory/Principal Ideal Domain]].
