...
$$
A family $(\mathcal{A}_{i})$ of sub-[[Analysis/sigma-algebra]]s of $\mathcal{F}$ is independent
if and only if the family $(A_{i})_{i\in I}$ is independent whenever $A_{i}\in \mathcal{A}_{i}$ for all $i$.

### Theorem
Let $\mathcal{A}_{1}$ and $\mathcal{A}_{2}$ be [[Analysis/pi-system]]s contained in $\mathcal{F}$ and suppose that 
$$
