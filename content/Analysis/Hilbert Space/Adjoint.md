Let $\mathcal{H}$ be a [[Analysis/Hilbert Space/Hilbert Space]] and $T\in L(\mathcal{H})$ a [[Analysis/Linear Operator]]. Then there exists a unique $T^{*}:\mathcal{H}\to \mathcal{H}$ (sometimes $T^{\dagger}$) such that 
$$
(Tx,y)=(x,T^{*}y)
$$
for all $x,y\in \mathcal{H}$
Moreover, $T^{*}\in L(\mathcal{H})$
If $T$ has a [[Analysis/Hilbert Space/Matrix of a Linear Operator]] $A$ with respect to some [[Analysis/Orthonormal Basis]], then $T^{*}$ has matrix $\overline{A^{T}}$
### Proposition
For $\mathcal{H}$ a [[Analysis/Hilbert Space/Hilbert Space]], $S,T\in L(\mathcal{H})$, have:
