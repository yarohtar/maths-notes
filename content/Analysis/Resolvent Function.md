The function $\lambda \to(\lambda I-T)^{-1}$ from the [[Analysis/Resolvent Set]] $\mathbb{C}\setminus\sigma(T)$ to $L(X)$ is the resolvent function of $T$ written $R_{T}$.

For $\lvert \lambda \rvert>\lVert T \rVert$ have $R_{T}(\lambda)=\sum_{n=0}^{\infty} \frac{T^{n}}{\lambda^{n+1}}$ by the following theorem
### Theorem
Let $X$ be a [[Analysis/Banach Space]] and $T\in L(X)$. 
