...
If $\sum_{n}\mu(A_{n})<\infty$, then $\mu(\lim\sup A_{n})=0$ ([[Analysis/lim sup]])
#### Proof
As $n\to \infty$
$$
...
Assume that the events $(A_{n})$ are [[Probability/Independence\|independent]]. 
If $\sum_{n}\mathbb{P}(A_{n})=\infty$ then $\mathbb{P}(\lim\sup A_{n})=1$. ([[Analysis/lim sup]])
