...
1. $x\to f(t,x)$ is [[XNotes/Integral\|integrable]] for all $t\in U$
2. $t\to f(t,x)$ is differentiable for all $x$
3. for some [[XNotes/Integral\|integrable]] $g$ and for all $x\in E$, $t\in U$
   $$
\left\lvert  \frac{\partial f}{\partial t}(t,x)  \right\rvert \leq g(x)
$$
Then the function $x\to(\partial f/ \partial t)(t,x)$ is [[XNotes/Integral\|integrable]] for all $t$. 
