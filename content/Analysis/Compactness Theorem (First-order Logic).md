Let $S$ be a first-order [[XNotes/Theory]].
...
As proofs are finite, there's a finite $S'\subseteq S$ s.t. $S'\vdash \bot$. Then by the [[XNotes/Soundness Theorem (First-order Logic)]] $S'\models \bot$ i.e. $S'$ has no model.
