...
#### Proof
If $S\models \bot$ then $S\vdash \bot$ by [[Foundations/First Order Logic/Adequacy Theorem (First-order Logic)]]
As proofs are finite, there's a finite $S'\subseteq S$ s.t. $S'\vdash \bot$. Then by the [[Foundations/First Order Logic/Soundness Theorem (First-order Logic)]] $S'\models \bot$ i.e. $S'$ has no model.
