Let $C$ be a [[XNotes/Quantum Circuit]] which is equivalent to $CNOT\circ H\otimes I$
...
We can also generate all the Bell states from $\ket{\phi^{+}}$ using [[XNotes/Pauli Matrices]]:
$$
\ket{\phi^{-}} = Z \ket{\phi^{+}}
$$
$$
