...
We try to use [[Quantum/Computing/Periodicity Determination]] on a function $f(x)=a^{x}\pmod{N}$ finding some smallest period $r$.
...
We try to implement [[Quantum/Computing/Periodicity Determination]] of $f(x)=a^{x}\pmod{N}$ on a whole domain $\mathbb{Z}_{M}$.
...
We find the [[Algebra/Number Theory/Continued Fraction]] of $\frac{c_{n}}{2^{m}}$. 
