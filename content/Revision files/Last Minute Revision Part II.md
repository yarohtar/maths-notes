...
### Proof
[[Algebra/Vector Spaces/The Krylov Subspaces]]
...
We find $m$ by [[Algebra/Number Theory/Chinese Remainder Theorem]]
...
## Pauli
Act on $\mathcal{H}=\mathbb{C}^{2}$ (single [[Quantum/Computing/Qubit]])
$$
...
#### Properties
- They are [[Analysis/Hilbert Space/Hermetian]] and unitary
- They anticommute $\{ X,Y \}=0$
- They are traceless
- $\sigma_{i}\sigma_{j}=i\varepsilon_{ijk}\sigma_{k}$ (IMPORTANT PLS REMEMBER)
In standard basis:
- $X\ket{0}=\ket{1}$, $X\ket{1}=\ket{0}$ for [[Quantum/Computing/Qubit]] (bitflip operator)
...
Bob also has a particle $B$ and $AB$ is in state $\ket{\phi^{+}}$ [[Quantum/Computing/Bell basis]]
