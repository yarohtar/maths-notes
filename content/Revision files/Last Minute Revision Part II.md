...
### Theorem
If $A$ is [[Analysis/Strictly diagonally dominant]], then the Jacobi method converges.
#### Proof
Note $e^{k+1}=He^{k}$ so $e^{k}=H^{k}e^{0}$
Thus we need to prove $\rho(H)<1$
Let $\lambda$ be an eigenvalue of $H$.
...
Now use [[Numerical/The Householder-John Theorem]] to find $\rho(H)<1$
...
## Watson's lemma
[[Asymptotic Methods/Watson's lemma]]
Consider 
$$
...
## Laplace Method
[[Asymptotic Methods/Laplace Method]]
$$
...
## Steepest descent
[[Asymptotic Methods/Method of Steepest Descent]]
...
## Linear codes
### Parity check matrix
An $(n,k)$ [[Information Theory/Linear Code]] with [[Information Theory/Generator Matrix]] $G=(I_{k}|B)$ 
...
3. $RM(d,r)=RM(d-1,r)\mid RM(d-1,r-1)$ [[Information Theory/The Bar Product]]
...
\end{pmatrix}
$$
Taking any $\delta-1$ columns, gives a [[Algebra/Vandermonde matrix]], 
...
\max_{X} I(X;Y)
$$
(where $I$ is [[Information Theory/Mutual Information]])
...
(i.e. they have [[Information Theory/Information Rate]] $\lim_{n\to \infty}\rho(C_{n})=R$)
such that [[Information Theory/Maximum Error Probability]]
$$
...
## Pauli
Act on $\mathcal{H}=\mathbb{C}^{2}$ (single [[Quantum/Qubit]])
$$
...
#### Properties
- They are [[Analysis/Hermetian]] and unitary
- They anticommute $\{ X,Y \}=0$
- They are traceless
- $\sigma_{i}\sigma_{j}=i\varepsilon_{ijk}\sigma_{k}$ (IMPORTANT PLS REMEMBER)
In standard basis:
- $X\ket{0}=\ket{1}$, $X\ket{1}=\ket{0}$ for [[Quantum/Qubit]] (bitflip operator)
...
- [[Quantum/Hilbert-Schmidt Inner Product]] satisfies $\langle A,B \rangle_{HS}=0$ for $A\neq B$ both from above basis
...
There exists a [[Combinatorics/Matching]] from $X$ to $Y$ if and only if 
...
The minimum size of $a-b$ [[Combinatorics/Separator]] is the maximum number 
...
## Proposition
Let $G /e$ be the [[Combinatorics/Contraction along edge]] of $G$ 
Then 
$$
