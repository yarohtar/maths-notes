...
- Newton error is less than ${2m^3\over L^2}({L\over 2m^2}||Df(x_0)||)^{2^{k+1}}$ where again $D^2f\geq mI$ and $D^2f$ is $L$-[[Analysis/Lipschitz]]. Sadly, in newton we need to recompute the gradient every step which can be a pain.
...
Now apply [[Numerical/Optimality conditions in LPs#Optimality condition\|the optimality condition]], noting that (in that notation) $c_B=0$ (and $B=I$), hence we only need to check $c\geq 0$. If so, we are done.
...
First consider the problem of minimizing $\sum_i y_i$ s.t. $Ax+y=b$, with $x,\ y\geq 0$. Now this is of the form $(A\quad I)y=b$, so we can apply the [[Numerical/Simplex method#Simplex algorithm\|simplex algorithm]]. This *has to* terminate at a solution $(x_0^T\quad 0^T)$ if the original problem is feasible. But now $x_0$ is a [[Numerical/Basic solutions (LP)\|BFS]] of the original problem. Now, given that we also tracked the $c^Tx$ function in this algorithm, we are sure to have a good setup for Phase 2.
### Phase 2
We can remove $y$ from our consideration and apply the [[Numerical/Simplex method#Simplex algorithm\|simplex algorithm]] again, now only looking at $c^Tx$.
...
${\partial f\over \partial \dot x} = \text{const}$ which is a First integral in [[Calculus/Euler-Lagrange#First integrals]].
...
## Second variation
Integrate by parts ($\int Q\eta^2+P\dot\eta^2$) to find a [[Calculus/Sturm-Liouville operator]] and then solve $\mathcal L \eta =-\omega^2\eta$. 
# Numerical Analysis
...
Then using [[Analysis/Taylor's theorem]]:
...
## Linear stability
Finding a solution to $y'=\lambda y$ often ends up with $y_n\sim (r(\lambda h))^n$. Hence, for A-stability, we only need to check that the maximum of $|r(\lambda h)|$ is $<1$ on $\mathbb C^-$. Use [[Analysis/Maximum principle]], so we only need to check $|r(z)|\leq 1$ on imaginary axis. 
## QR factorisation
### Gram-Schimdt
...
Let $R$ be the radius of convergence of a power series. Then for any $0<r<R$ the series [[Analysis/Uniform convergence of functions\|converges uniformly]] on $D(a,r)$.
#### Proof sketch
...
So take $M_n=M{r^n\over |w-a|^n}$ and note that $\sum_{n=1}^\infty M_n$ converges. We are done by [[Analysis/Uniform Cauchy]].
