...
2. If $X$ is [[Topology/Hausdorff]] and $Y$ is compact, then $Y$ is closed
#### Proof sketch
...
2. We show $X\setminus Y$ is open. Let $x\in X$. For any $y\in Y$ find open $U_y$ and $V_y$ disjoint s.t. $x\in U_y$ and $y\in V_y$ (because $X$ is [[Topology/Hausdorff]]). Now, certainly $\{Y\cap V_y\}$ makes an open cover of $Y$, so find a finite subcover and let $F$ be the finite set of $y$s s.t. $\{Y\cap V_y:y\in F\}$ makes this finite subcover. Then take $U=\bigcap_{y\in F}U_y$. This is an open nbd of $x$ disjoint from $Y$. 