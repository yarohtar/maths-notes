...
2. [[Analysis/Hausdorff space]]
3. $A\subset X$ is open if every point has an open [[Analysis/Neighbourhood\|nbd]] that is contained in $A$
4. $x_n\to x$ if for any nbd $V$ of $x$ we find $N$ s.t. $x_n\in V$ for $n>N$
5. In a [[Analysis/Hausdorff space]], the limits are unique.
...
9. [[Analysis/Interior and closure]]
10. [[Topology/Subspace topology]]
11. [[Analysis/Continuity in topological spaces]]
12. $X$ and $Y$ are [[Analysis/Homeomorphism\|homeomorphic]] if there is a bijection $f$ between them such that both $f$ and $f^{-1}$ are continuous
13. [[Analysis/Product topology]]
14. [[Analysis/Open map]]
15. [[Analysis/Quotient space]]
