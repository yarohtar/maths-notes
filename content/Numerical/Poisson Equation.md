...
We use [[Numerical/Finite difference\|finite differences]] to discretize the equation. 
$$
...
Now by [[Analysis/Gershgorin Theorem]] the eigen values are at most $0$ 
...
Note that $B$ is a [[Algebra/Vector Spaces/TST matrix]] and so we can diagonalize it, 
...
Now apply [[Analysis/Fast Fourier Transform]] to do it in $O(m\log m)$.
