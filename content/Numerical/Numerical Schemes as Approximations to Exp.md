...
(For example, by [[Numerical/Semidiscretization]])
...
### Euler
$e^{z} = 1+z+O(z^{2})$ gives [[Numerical/Euler method]]
$$
u^{n+1} = (I+kA) u^{n}
$$
### Implicit Euler
$e^{z}=\frac{1}{1-z} + O(z^{2})$ gives [[Numerical/Reverse-Euler Method]]
$$
u^{n+1}=(I-kA)^{-1}u^{n}
$$
### Trapezoidal
$e^{z}= \frac{1+z /2}{1- z /2} + O(z^{3})$ gives [[Numerical/Trapezoidal rule (ODEs)]]
$$
