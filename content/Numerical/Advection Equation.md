...
\end{pmatrix}
$$
But $A$ is not [[Analysis/Normal Matrix]]. 
...
We use [[Numerical/Semidiscretization]]:
$$
\frac{du_{m}}{dt} = \frac{1}{2h} (u_{m+1} - u_{m-1})
$$
and [[Numerical/Trapezoidal rule (ODEs)]]
$$
...
where $B$ and $C$ are [[Algebra/Vector Spaces/TAntiST Matrix]] and hence [[Analysis/Normal Matrix]]
$$
B=\begin{pmatrix}
1 & -\frac{1}{4}\mu &  \\
\frac{1}{4}\mu & 1 & \ddots &  \\
...
$$
$A$ is also [[Analysis/Normal Matrix]] so $\rho(A)=1$, thus the method is stable.
