### Lemma
Let $p:\tilde{X}\to X$ be a [[XNotes/Covering Spaces\|covering map]] with $x_{0}\in X$, $\tilde{x}_{0}\in p ^{-1}(x_{0})$.
Then $p_{*}:\pi_{1}(\tilde{X},\tilde{x}_{0})\to \pi_{1}(X,x_{0})$ is injective 
(where $p_{*}$ is the natural [[XNotes/Fundamental group isomorphisms\|homomorphism]] induced by $p$).
...
$$
where $\gamma_{*}$ is the [[XNotes/Path Push Forward]] of $\gamma$.
### Lemma
...
### Corollary
Suppose $p:\tilde{X}\to X$ is a [[XNotes/Universal Cover\|universal cover]]. 
