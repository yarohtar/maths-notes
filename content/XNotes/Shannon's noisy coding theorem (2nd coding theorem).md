[[XNotes/Operational Channel Capacity]] = [[XNotes/Information Channel Capacity]]
...
$$
#### Proof
Let $C$ be the [[XNotes/Information Channel Capacity]]. Suppose we can [[XNotes/Transmit Reliably]] at rate $R>C$ i.e. there is a sequence of codes $(C_{n})_{n\geq 1}$ with $C_{n}$ of length $n$ and size $\lfloor 2^{nR} \rfloor$ s.t. $\hat{e}(C_{n})\to 0$.
...
So, $e(C_{n})=P(X\neq Y)=p$ say
Then by [[XNotes/Fano's Inequality]] write:
$$
\begin{align}
