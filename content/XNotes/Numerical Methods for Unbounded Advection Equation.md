...
Consider the [[XNotes/Semidiscretization]]:
$$
\frac{\partial u_{m}(t)}{\partial t} = \frac{1}{h} (u_{m}(t)-u_{m-1}(t))
$$
using the [[XNotes/Euler method]]:
$$
u_{m}^{n+1}=u_{m}^{n}+\mu(u_{m}^{n}-u_{m-1}^{n})
$$
for $n\in \mathbb{Z}_{+}$
We use [[XNotes/Fourier Analysis of Stability]]:
$$
...
## Leap-frog method
We [[XNotes/Semidiscretization]] as
$$
...
We use [[XNotes/Fourier Analysis of Stability#Multi-step]]:
$$
