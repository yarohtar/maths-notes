...
(For example, by [[XNotes/Semidiscretization]])
...
### Euler
$e^{z} = 1+z+O(z^{2})$ gives [[XNotes/Euler method]]
$$
u^{n+1} = (I+kA) u^{n}
$$
### Implicit Euler
$e^{z}=\frac{1}{1-z} + O(z^{2})$ gives [[XNotes/Reverse-Euler Method]]
$$
u^{n+1}=(I-kA)^{-1}u^{n}
$$
### Trapezoidal
$e^{z}= \frac{1+z /2}{1- z /2} + O(z^{3})$ gives [[XNotes/Trapezoidal rule (ODEs)]]
$$
