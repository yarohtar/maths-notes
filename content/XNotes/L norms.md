...
#### On sequences
Let $S$ be the space of all scalar sequences. We define the [[XNotes/Normed Space\|normed space]] 
$$
l_{p}=\left\{  X\in S : \sum_{i=1}^{\infty} |X_{i}|^p<\infty  \right\}
$$
With norm $||X||_{p}=\left( \sum_{i=1}^{\infty}|X_{i}|^p \right)^{\frac{1}{p}}$.

These all satisfy the desired properties by [[XNotes/Minkowski's Inequality\|Minkowski]] 
