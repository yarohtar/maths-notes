...
\end{pmatrix}
$$
But $A$ is not [[XNotes/Normal Matrix]]. 
...
We use [[XNotes/Semidiscretization]]:
$$
\frac{du_{m}}{dt} = \frac{1}{2h} (u_{m+1} - u_{m-1})
$$
and [[XNotes/Trapezoidal rule (ODEs)]]
$$
...
where $B$ and $C$ are [[XNotes/TAntiST Matrix]] and hence [[XNotes/Normal Matrix]]
$$
B=\begin{pmatrix}
1 & -\frac{1}{4}\mu &  \\
\frac{1}{4}\mu & 1 & \ddots &  \\
...
$$
$A$ is also [[XNotes/Normal Matrix]] so $\rho(A)=1$, thus the method is stable.
