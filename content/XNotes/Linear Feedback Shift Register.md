...
[[XNotes/Feedback Shift Register]] $f:\mathbb{F}_{2}^{d}\to \mathbb{F}_{2}^{d}$ with $C$ linear i.e. 
$$
...
$$
[[XNotes/Generating Function]]

### Theorem
The stream $(y_{n})_{n\in \mathbb{N}}$ comes from a linear feedback shift register
with auxiliary polynomial $P$ if and only if its [[XNotes/Generating Function]] 
