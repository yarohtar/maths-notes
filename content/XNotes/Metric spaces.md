...
A [[XNotes/Lipschitz]] function is [[XNotes/Uniform continuity\|uniformly continuous]].

On $(M,d)$ for $x\in M$ and $r<0$ we define the open ball $D_r(x)=\{y\in M: d(x,y)<r\}$ and closed ball $B_r(x)=\{y\in M: d(x,y)\leq r\}$

A set $A$ is open iff every point in $A$ has an open ball contained in $A$. This is the metric induced [[XNotes/Topological spaces\|topology]].
...
[[XNotes/Picard-Lindelof Theorem]]
