...
Note that [[XNotes/Watson's lemma]] deals with the special case $\phi=-t$ 
...
$$
Can use [[XNotes/Watson's lemma]] or just use partial integration:
$$
...
\end{align}
$$
From the [[XNotes/Laplace integrals#Case monotonic]].
...
$$
(this is done by doing the same manipulation as in [[XNotes/Laplace integrals#Case one max]] but the last integral is from $-\infty$ to $0$)
