[[XNotes/Expected word length]] for an [[XNotes/Optimal Code]] $f$ satisfies:
$$
\frac{H(X)}{\log a}\leq E(S)<\frac{H(X)}{\log a}+1
$$
where $a=\lvert \Sigma_{2} \rvert$, and $H(X)$ is the  [[XNotes/Mathematical Entropy]].

#### Proof
The lower bound is given by combining [[XNotes/Gibbs' inequality]] and [[XNotes/Kraft's inequality]].
...
\end{align}
$$
Where $\log C\leq 0$ by [[XNotes/Kraft's inequality]].
Hence:
$$
...
$$
So by [[XNotes/McMillan's Theorem]] there is a prefix-free code with word lengths $s_{1},\dots,s_{m}$
Also
$$
\begin{align}
