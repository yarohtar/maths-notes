...
This is a [[XNotes/Sturm-Liouville operator]] $\mathcal L$, so if $\mathcal L(\eta)=-\omega^2\eta$ has a solution for some real $\omega$, we find $\delta^2F[x](\eta)<0$. Note that $\eta$ here still HAS to satisfy the 0 boundary conditions.
### Example
...
## Ashton alternative
Let $\mathcal L=-{d\over dt}(P{d\over dt})+Q$ be a [[XNotes/Sturm-Liouville operator]] and find it's eigenvalues $\lambda_n$ with eigenfunctions $\psi_n$. Then for any $\eta$ satisfying boundary conditions we can write:
$$\eta = \sum_n c_n\psi_n$$
Now note that
$$\begin{align*}
