...
is the classic [[XNotes/Gram-Schmidt process]], where
$$b_j=a_j-\sum_{i=1}^{j-1}{R_{ij}\over ||b_i||}b_i$$
with $q_i={b_i\over ||b_i||}$ and $R_{ij}=\braket{ q_{i} | a_{j} }$ for $i<j$ and $R_{jj}=||b_j||$.

## Givens rotation
Use [[XNotes/Givens Rotation]] to introduce $0$s from left to right.
## Householder reflections
Take a [[XNotes/Householder Reflection]] $H_{u}$
where $u=a-b$ for 
