Let $(\Omega,\mathcal{F},\mathbb{P})$ be a [[XNotes/Probability Space]] and $I$ a countable set. 
...
$$
A family $(\mathcal{A}_{i})$ of sub-[[XNotes/sigma-algebra]]s of $\mathcal{F}$ is independent
if and only if the family $(A_{i})_{i\in I}$ is independent whenever $A_{i}\in \mathcal{A}_{i}$ for all $i$.

### Theorem
Let $\mathcal{A}_{1}$ and $\mathcal{A}_{2}$ be [[XNotes/pi-system]]s contained in $\mathcal{F}$ and suppose that 
$$
