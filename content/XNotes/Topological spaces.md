...
1. Some topologies are induced by a [[XNotes/Metric spaces\|metric]].
2. [[XNotes/Hausdorff space]]
3. $A\subset X$ is open if every point has an open [[XNotes/Neighbourhood\|nbd]] that is contained in $A$
4. $x_n\to x$ if for any nbd $V$ of $x$ we find $N$ s.t. $x_n\in V$ for $n>N$
5. In a [[XNotes/Hausdorff space]], the limits are unique.
...
8. [[Analysis/Accumulation points]]
9. [[XNotes/Interior and closure]]
10. [[XNotes/Subspace topology]]
11. [[Analysis/Continuity in topological spaces]]
12. $X$ and $Y$ are [[XNotes/Homeomorphism\|homeomorphic]] if there is a bijection $f$ between them such that both $f$ and $f^{-1}$ are continuous
13. [[XNotes/Product topology]]
14. [[XNotes/Open map]]
15. [[XNotes/Quotient space]]

## More stuff
[[Analysis/Connectedness]]
[[Analysis/Compactness]]
[[XNotes/Sequential compactness]]