...
Let $[c,d]=[t_0-\epsilon,t_0+\epsilon]\cap [a,b]$. The metric space $M=C([c,d],B_R(y_0))$ (of [[Analysis/Continuous function spaces\|continuous functions]]) is complete in uniform metric. Hence define $T:M\to M$
...
&\leq |g(x)-h(x)|/2
\end{align}$$
Hence, we are done by [[Analysis/Contraction mapping\|contraction mapping theorem]] and FTA (i.e. $(Tg)=g\iff g$ is the solution to original problem).