...
Thus $f(z)$ is an [[XNotes/Upper Bound]] for $S$ and so $z\leq f(z)$
...
A [[XNotes/Relation Class]] $r$ is local if 
...
Let $r$ be [[XNotes/Well-Founded]], [[XNotes/Extensional]] relation on a set $a$.
(Note that this implies $r$ is [[XNotes/Local]])
...
### Note
Similar to [[XNotes/Epsilon-recursion Theorem]] and [[XNotes/Principle of Epsilon-Induction]] 
...
## Ordinal
An ordinal is a [[XNotes/Transitive]] set well ordered by $\in$.
Order types are defined and unique for all sets 
by [[XNotes/Mostowski's Collapsing Theorem]]
## Cardinal
...
### Theorem
If $A$ is [[XNotes/Strictly diagonally dominant]], then the Gauss-Seidel method converges.
#### Proof
...
Use [[XNotes/The Householder-John Theorem]] to find $\rho(H)<1$
...
# The method
See [[XNotes/Standard Conjugate Gradient Method]] for the optimized algorithm.
### Initial conditions
...
Note that we will need [[XNotes/LU factorisation]] of $A$
This is very efficient if $A$ is [[XNotes/Upper Heisenberg]]

We can bring $A$ in this form using [[XNotes/Givens Rotation]] or [[XNotes/Householder Reflection]]

### QR
Set $A_{0}=A=Q_{0}R_{0}$ [[XNotes/QR factorisation]]
Then iterate as:
$$
...
$$
The first column of $A^{k+1}$ is related to [[XNotes/Power Method for Finding Eigenvalues]] 
The last row of $A^{k+1}$ is related to [[XNotes/Inverse Iteration for Finding Eigenvalues]]
...
## Steepest descent
[[XNotes/Method of Steepest Descent]]
...
## Linear codes
### Parity check matrix
An $(n,k)$ [[XNotes/Linear Code]] with [[XNotes/Generator Matrix]] $G=(I_{k}|B)$ 
...
### Reed Muller
Let $C_{1}$, $C_{2}$ be [[XNotes/Linear Code]] of length $n$ with $C_{2}\subseteq C_{1}$ 
The bar product is:
$$
...
### Theorem
The [[XNotes/Minimum distance of a code]] for BCH code is at least the design distance $\delta$.
#### Proof
Consider the matrix:
$$
H=\begin{pmatrix}
...
\end{pmatrix}
$$
Taking any $\delta-1$ columns, gives a [[XNotes/Vandermonde matrix]], 
...
Note that $H$ is not the [[XNotes/Parity Check Matrix]] 
...
Consider a [[Information Theory/Discrete Memoryless Channel (DMC)]] 
...
\max_{X} I(X;Y)
$$
(where $I$ is [[XNotes/Mutual Information]])
...
A [[Information Theory/Communication Channel]] can transmit reliably at rate $R$ if 
there is a sequence of [[Information Theory/Binary Code]]s $C_{1},C_{2},\dots$ 
where $C_{n}$ is a code of length $n$ and size $\lfloor 2^{nR} \rfloor$ 
(i.e. they have [[XNotes/Information Rate]] $\lim_{n\to \infty}\rho(C_{n})=R$)
such that [[XNotes/Maximum Error Probability]]
$$
...
For a [[Information Theory/Binary Code]] $C$, we define
$$
\hat{e}(C)=\max_{c\in C}P(\text{error}|c \text{ sent})
$$
the maximum error probability.

Note that this is the error of decoding wrong, not the error of the [[Information Theory/Communication Channel]].
...
We find $m$ by [[Algebra/Chinese Remainder Theorem]]
...
## Pauli
Act on $\mathcal{H}=\mathbb{C}^{2}$ (single [[XNotes/Qubit]])
$$
...
#### Properties
- They are [[XNotes/Hermetian]] and unitary
- They anticommute $\{ X,Y \}=0$
- They are traceless
- $\sigma_{i}\sigma_{j}=i\varepsilon_{ijk}\sigma_{k}$ (IMPORTANT PLS REMEMBER)
In standard basis:
- $X\ket{0}=\ket{1}$, $X\ket{1}=\ket{0}$ for [[XNotes/Qubit]] (bitflip operator)
...
- [[XNotes/Hilbert-Schmidt Inner Product]] satisfies $\langle A,B \rangle_{HS}=0$ for $A\neq B$ both from above basis
- Also $\langle A,A \rangle_{HS}=2$

## Teleport
Suppose Alice has a particle in state $\ket{\psi}$ and a particle $A$
Bob also has a particle $B$ and $AB$ is in state $\ket{\phi^{+}}$ [[Quantum/Bell basis]]
...
There exists a [[XNotes/Matching]] from $X$ to $Y$ if and only if 
...
The minimum size of $a-b$ [[XNotes/Separator]] is the maximum number 
...
## Proposition
Let $G /e$ be the [[XNotes/Contraction along edge]] of $G$ 
Then 
$$
