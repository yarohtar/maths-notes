Let $S$ be a [[Foundations/Consistent]] [[XNotes/Theory]] in a first-order language $L=L\left( \Omega,\prod \right)$
Then $S$ has a model.
...
We turn this into a [[XNotes/Structure]] in the obvious way
...
1. Let $S$ be a consistent theory, $p$ be a sentence
   ...
   By [[XNotes/Zorn's Lemma]] there is a maximal consistent theory $\bar{S}\supseteq S$. 
...
We make $A$ an $L$ [[XNotes/Structure]]:
