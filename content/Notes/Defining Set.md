Let $C$ be a [[Notes/Cyclic Code]] of length $n$ (where $n$ is odd)
...
We say a [[Notes/Cyclic Code]] $C$ is defined by $A\subseteq \{ x\in K :x^{n}=1\}$ if:
$$
C=\{ f\in F[X] /(X^{n}-1) : (\forall a\in A)\ f(a)=0 \}
$$
### Remark
We usually work with $K$ the [[Notes/Splitting Field]] of $X^{n}-1$ 
(which can be shown to be $\mathbb{F}_{2^{r}}$ where $2^{r}=1\pmod{n}$)



# This wasn't done in lectures like this
See [[Notes/Cyclic Code with Defining Set]]