...
Huffman [[Notes/Code]] is defined inductively:
...
Let $f$ be an [[Notes/Optimal Code]], prefix free, with word lengths $s_{1},\dots ,s_{m}$
...
$m>2:$ [[Notes/Source]] $X_{m}$ emits $\mu_{1},\dots,\mu_{m}$ with probabilities $p_{1}\geq\dots\geq p_{m}$
