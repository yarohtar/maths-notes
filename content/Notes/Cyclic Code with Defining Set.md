Let $K$ be some [[Notes/Finite Field]] extension of $\mathbb{F}_{2}$ and $n$ odd
Let $A\subseteq \{ x\in K : x^{n}=1 \}$
The [[Notes/Cyclic Code]] of length $n$ with defining set $A$ is the code:
$$
C=\{ f(X)\in \mathbb{F}_{2}[X] /(X^{n}-1): (\forall a\in A)\ f(a)=0 \}
$$
Note that as the [[Notes/Generator Polynomial]] $g(X)$ of $C$ divides $X^{n}-1$, 
...
We also find that [[Notes/Splitting Field]] of $X^{7}-1$ is $\mathbb{F}_{8}\simeq \mathbb{F}_{2} /(X^{3}+X+1)$
