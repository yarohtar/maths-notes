### Definition
Having a [[Notes/Paths\|path]] between two points is an equivalence relation and the equivalence classes are called the path components of $X$.
...
### Corollary
If $f:X\to Y$ is a [[Notes/Homotopy#Definition (homotopy-equivalence)\|homotopy equivalence]] then $\pi_{0}(f):\pi_{0}(X)\to \pi_{0}(Y)$ is a bijection.
