The minimum distance of a [[Notes/Binary Code]] $C$ is the smallest [[Notes/Hamming distance]] between distinct codewords.

An $[n,m]$ [[Notes/Binary Code]] with minimum distance $d$ is sometimes called $[n,m,d]$

### Lemma
Let $C$ be a code with minimum distance $d$ 
1. Then $C$ is $(d-1)$-[[Notes/Error detecting]], but cannot detect $d$ errors
2. $C$ is $\left\lfloor  \frac{d-1}{2}  \right\rfloor$-[[Notes/Error correcting]], but cannot correct all sets of $\left\lfloor  \frac{d-1}{2}  \right\rfloor+1$ errors
#### Proof
