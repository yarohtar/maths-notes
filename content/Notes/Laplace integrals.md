...
Note that [[Notes/Watson's lemma]] deals with the special case $\phi=-t$ 
(and $a=0$, $b=T$, so the endpoint dominates the integral)

To see a systematic way to find an [[Notes/Asymptotic Approximation]]
see [[Notes/Laplace Method]]
We give some examples below:
### Principle of localisation
The [[Notes/Asymptotic Expansion]] of $I$ as $x\to \infty$ is determined by 
...
$$
Can use [[Notes/Watson's lemma]] or just use partial integration:
$$
...
\end{align}
$$
From the [[Notes/Laplace integrals#Case monotonic]].
...
$$
(this is done by doing the same manipulation as in [[Notes/Laplace integrals#Case one max]] but the last integral is from $-\infty$ to $0$)
