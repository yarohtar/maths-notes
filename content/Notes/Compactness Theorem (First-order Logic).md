Let $S$ be a first-order [[Notes/Theory]].
If every finite subset of $S$ has a model, then $S$ has a model.

#### Proof
If $S\models \bot$ then $S\vdash \bot$ by [[Notes/Adequacy Theorem (First-order Logic)]]
As proofs are finite, there's a finite $S'\subseteq S$ s.t. $S'\vdash \bot$. Then by the [[Notes/Soundness Theorem (First-order Logic)]] $S'\models \bot$ i.e. $S'$ has no model.
...
By [[Notes/Compactness]] $S$ has a model.
