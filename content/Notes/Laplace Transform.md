...
If additionally $f(t)=0$ for $t<0$, then $F(s)=-\tilde f(-is)$ ([[Notes/Fourier Transform]])
...
- Convolution: $\mathcal L\{f*g\}(s)=F(s)G(s)$, where $f*g$ is [[Notes/Convolution]] of these functions.
