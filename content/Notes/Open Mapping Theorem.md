$X$, $Y$ [[Notes/Banach Space]], $T\in L(X,Y)$ surjective. Then $T$ is an open mapping.
(ie $\exists k$ st $\forall y\in Y$ )
#### Proof
$T$ surjectivee
So $\bigcup_{n}\overline{T(nB_{x})}=Y$ with each $\overline{T(nB_{x})}$ closed.
Thus by [[Notes/The Baire Category Theorem]], some $\overline{T(n,B_{x})}$ contains a ball $B(y,\epsilon)$.
...
Thus $T$ any open mapping by [[Notes/Open Mapping Lemma]] applied to $\frac{T}{\epsilon}$.

### Corollaries
[[Notes/Inversion Theorem]]
[[Notes/Comparability Theorem]]