A DMC is a [[Notes/Communication Channel]] for which 
$$
P_{ij}=P(b_{j}\text{ received}\mid a_{i}\text{ sent})
$$
is the same each time the channel is used, it is independent of all past uses

[[Notes/Binary Symmetric Channel (BSC)]]
[[Notes/Binary Erasure Channel]]

### Lemma
The $n$-th [[Notes/Communication Channel Extension]] of a DMC with [[Notes/Information Channel Capacity]] $C$ has information capacity $nC$
#### Proof
