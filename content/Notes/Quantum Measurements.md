# General definition
Let $\mathcal{H}$ be a [[Notes/Hilbert Space]]. 
Consider a collection of [[Notes/Linear Operator]]s $\{ M_{n} \}_{n} \subseteq \mathcal{B(\mathcal{H})}$ with $\sum_{n}M_{n}^{*}M_{n}=\mathbb{1}$
...
(i.e. they are [[Notes/Hermetian]] and $M_{n}M_{m}=\delta_{mn}M_{n}$).
Now any [[Notes/Hermetian]] operator $M$ can be represented as 
$$
M = \sum_{n} \lambda_{n}M_{n}
$$
where projected spaces of $M_{n}$ correspond to eigenspaces of $M$ with eigenvalue $\lambda_{n}$
This defines the [[Notes/Observables]]
...
Then we define [[Notes/Positive semi-definite]] operators:
$$
E_{n} = M_{n}^{*}M_{n}
$$
...
- Any observable $O$ has associated [[Notes/Hermetian]] operator $\hat O$.
