...
# General method
We are minimizing $f(x)$ subject to $x\in X$ and $g(x)\leq b$. The following method works as long as we have [[Notes/The Necessity Theorem]].
...
8. Use [[Notes/Lagrange sufficiency theorem]] to finalize.
...
Now apply [[Notes/Lagrange sufficiency theorem]] to this.
...
That is a feasible solution so by [[Notes/Lagrange sufficiency theorem]] we are done.
...
This way, the [[Notes/Euler-Lagrange]] for the function $\lambda(t)$ will give condition $g(t,x,\dot x)=0$. 