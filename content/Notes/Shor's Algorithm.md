...
We try to use [[Notes/Periodicity Determination]] on a function $f(x)=a^{x}\pmod{N}$ finding some smallest period $r$.
...
We try to implement [[Notes/Periodicity Determination]] of $f(x)=a^{x}\pmod{N}$ on a whole domain $\mathbb{Z}_{M}$.
...
We find the [[Notes/Continued Fraction]] of $\frac{c_{n}}{2^{m}}$. 
