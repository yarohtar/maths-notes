...
This is of course a [[Notes/Linear programs\|linear program]].
...
This is optimal by the [[Notes/The max-flow min-cut Theorem]]. To prove there are no more augmented paths, we just need to find a cut with $c(S)=f(S,\overline S)$. 
...
(using notation from [[Notes/The max-flow min-cut Theorem]])
