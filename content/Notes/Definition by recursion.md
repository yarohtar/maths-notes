Let $X$ be a [[Notes/Well-ordered]] set and $Y$ be an arbitrary set.
Then for any [[Notes/Function]] $G:\mathcal{P}(X\times Y)\to Y$ there is unique $f:X\to Y$ such that
$$
\forall x\in X\quad %quad
f(x)=G(f|_{I_{x}})
$$
where $I_{x}$ is the [[Notes/Initial Segment]].

### Theorem
Say $h$ is an attempt if $h$ is a function $I\to Y$ 
where the domain of $h$ is an [[Notes/Initial Segment]] $I$ of $X$ and 
$$
...
We first show that if $h,h'$ are attempts, then $h(x)=h'(x)$ for all $x\in dom(h)\cap dom(h')$ by [[Notes/Proof by Induction]].
...
$$
Now just check that this is a [[Notes/Function]] and check that $f$ is an attempt.
