A code $C\subseteq \mathbb{F}_{2}^{n}$ is a cyclic code if it is [[Notes/Linear Code]] and 
$$
(a_{0},\dots,a_{n-1})\in C\implies(a_{1},\dots,a_{n-1},a_{0})\in C
$$
We identify $\mathbb{F}_{2}^{n}$ with the [[Notes/Ring]] $\mathbb{F}_{2}[X] / (X^{n}-1)$ via
$$
...
### Corollary
$C$ is a cyclic code of length $n$ if and only if $\mathcal{C}$ is an [[Notes/Ideal]] in $\mathbb{F}_{2}[X] / (X^{n}-1)$
We identify $C$ and $\mathcal{C}$.
### Lemma
$\mathbb{F}_{2}[X] /(X^{n}-1)$ is a [[Notes/Principal Ideal Domain]]
Moreover, each of its ideals is generated by some $g$ such that $g(X)\mid X^{n}-1$
#### Proof
Note that $\mathbb{F}_{2}[X] /(X^{n}-1)$ is [[Notes/Euclidean Domain]] 
so also [[Notes/Principal Ideal Domain]].
...
[[Notes/Generator Polynomial]]
[[Notes/Parity Check Polynomial]]
### Proposition
