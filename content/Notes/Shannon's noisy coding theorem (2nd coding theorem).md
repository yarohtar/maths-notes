[[Notes/Operational Channel Capacity]] = [[Notes/Information Channel Capacity]]
We prove a bunch of propositions leading up to this 
We only actually prove the equality for a [[Notes/Binary Symmetric Channel (BSC)]]. 

### Proposition
For a [[Notes/Discrete Memoryless Channel (DMC)]] 
$$
\text{operational capacity }\leq \text{ information capacity}
$$
#### Proof
Let $C$ be the [[Notes/Information Channel Capacity]]. Suppose we can [[Notes/Transmit Reliably]] at rate $R>C$ i.e. there is a sequence of codes $(C_{n})_{n\geq 1}$ with $C_{n}$ of length $n$ and size $\lfloor 2^{nR} \rfloor$ s.t. $\hat{e}(C_{n})\to 0$.
...
So, $e(C_{n})=P(X\neq Y)=p$ say
Then by [[Notes/Fano's Inequality]] write:
$$
\begin{align}
...
### Proposition
Consider a [[Notes/Binary Symmetric Channel (BSC)]] with error probability $p$. Let $R<1-H(p)$.
...
Using minimum distance [[Notes/Decoding rule]]
Let $m=\lfloor 2^{nR} \rfloor$
We pick an $[n,m]$ [[Notes/Binary Code]] $C$ at random 
...
$$
### Proposition
Consider a [[Notes/Binary Symmetric Channel (BSC)]] with error probability $p$. Let $R<1-H(p)$. Then there is a sequence of codes $(C_{n})_{n\geq 1}$ with $C_{n}$ of length $n$, size $\lfloor 2^{nR} \rfloor$ and $\hat{e}(C_{n})\to 0$ as $n\to \infty$ 
#### Proof
