Let $\mathcal{H}$ be a [[Notes/Hilbert Space]] and $T\in L(\mathcal{H})$ a [[Notes/Linear Operator]]. Then there exists a unique $T^{*}:\mathcal{H}\to \mathcal{H}$ (sometimes $T^{\dagger}$) such that 
$$
(Tx,y)=(x,T^{*}y)
$$
for all $x,y\in \mathcal{H}$
Moreover, $T^{*}\in L(\mathcal{H})$
If $T$ has a [[Notes/Matrix of a Linear Operator]] $A$ with respect to some [[Notes/Orthonormal Basis]], then $T^{*}$ has matrix $\overline{A^{T}}$
### Proposition
For $\mathcal{H}$ a [[Notes/Hilbert Space]], $S,T\in L(\mathcal{H})$, have:
