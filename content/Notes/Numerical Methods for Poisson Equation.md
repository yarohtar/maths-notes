...
We use [[Notes/Finite difference\|finite differences]] to discretize the equation. 
$$
...
Now by [[Notes/Gershgorin Theorem]] the eigen values are at most $0$ 
...
Note that $B$ is a [[Notes/TST matrix]] and so we can diagonalize it, 
...
Now apply [[Notes/Fast Fourier Transform]] to do it in $O(m\log m)$.
