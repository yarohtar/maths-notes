[[Notes/Discrete Memoryless Channel (DMC)]] which has $\Sigma_{1}=\Sigma_{2}=\{ 0,1 \}$ with probability $0\leq p\leq 1$ and [[Notes/Channel Matrix]]
$$
\begin{pmatrix}
1-p & p \\
p & 1-p
...
### Lemma
Let $\epsilon>0$. A [[Notes/Binary Symmetric Channel (BSC)]] with error probability $p$ is used to send $n$ digits. Then
$$
...
### Proposition
The capacity of a [[Notes/Binary Symmetric Channel (BSC)]] with error probability $p<\frac{1}{4}$ is $\neq 0$
