After receiving a [[Notes/Binary Code]] through a [[Notes/Binary Symmetric Channel (BSC)]], we need to decode it.

There are 3 rules we consider when decoding.
Due to the lemma at the bottom, we shall use the [[Notes/Decoding rule#Minimum distance]] decoding rule.
## Ideal Observer
...
---
## Minimum distance
The minimum distance decoding rule decodes $x\in \{ 0,1 \}^{n}$ as $c\in C$ minimising the [[Notes/Hamming distance]] $d(x,c)$.
