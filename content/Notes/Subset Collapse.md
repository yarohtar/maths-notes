Let $Y$ be a [[Notes/Well-ordered]] set and $X\subseteq Y$
Then there is a unique [[Notes/Initial Segment]] of $Y$ which is [[Notes/Order-isomorphic]] to $X$ 
...
$$
for all $x\in X$
By [[Notes/Proof by Induction]], $f$ is uniquely determined 
##### Existence
WLOG $Y\neq \emptyset$
Fix $y_{0}\in Y$
...
It will then follow that $im(f)$ is an [[Notes/Initial Segment]] of $Y$ and we are done.
