[[Notes/Expected word length]] for an [[Notes/Optimal Code]] $f$ satisfies:
$$
\frac{H(X)}{\log a}\leq E(S)<\frac{H(X)}{\log a}+1
$$
where $a=\lvert \Sigma_{2} \rvert$, and $H(X)$ is the  [[Notes/Mathematical Entropy]].

#### Proof
The lower bound is given by combining [[Notes/Gibbs' inequality]] and [[Notes/Kraft's inequality]].
...
\end{align}
$$
Where $\log C\leq 0$ by [[Notes/Kraft's inequality]].
Hence:
$$
...
$$
So by [[Notes/McMillan's Theorem]] there is a prefix-free code with word lengths $s_{1},\dots,s_{m}$
Also
$$
\begin{align}
