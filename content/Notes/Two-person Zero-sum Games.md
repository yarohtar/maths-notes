...
Introduce $e=(1 \quad \dots \quad1)^T$ and reframe this into a [[Notes/Linear programs\|linear program]]:
maximise $v$ s.t. $A^Tp\geq ve$, $p\geq 0$, $e^Tp=1$.

[[Notes/Lagrange multipliers\|Lagrangian]] is 
...
Also in an optimal strategy $w=v$ (by [[Notes/Duality (Optimisation)#Strong duality\|strong duality]]). Hence, using [[Notes/Optimality conditions in LPs\|optimality conditions]] we arrive at the following theorem:
## Theorem
